...
void rtpCodecInitialize_audio(demuxer_t* demuxer,
  ...
  } else if (strcmp(subsession->codecName(), "SPEEX") == 0) {
//    wf->wFormatTag = sh_audio->format = 0xA109;
    wf->wFormatTag = sh_audio->format = mmioFOURCC('s','p','x',' ');

//						init_best_audio_codec(sh_audio, NULL, NULL);
						//	demuxer = init_avi_with_ogg(demuxer);
							
							/*
							stream_t* s;
							demuxer_t  *od;
							s = new_ds_stream(demuxer->audio);   //demuxer->stream<---fuckno;
							od = demux_open(s,DEMUXER_TYPE_OGG,-1,-2,-2,NULL);
							if(!demux_ogg_open(od)) {
								mp_msg( MSGT_DEMUXER,MSGL_ERR,"Fuck! Unable to open the Ogg demuxer.\n");
								free_stream(s);
								demuxer->audio->id = -2;
							} //else
